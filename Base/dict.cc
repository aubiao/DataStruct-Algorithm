...
int _dictExpand(dict *d, unsigned long size, int *malloc_failed) {
    ...
    if (malloc_failed) {
        new_ht_table = (dictEntry **) ztrycalloc(newsize * sizeof(dictEntry *));
        *malloc_failed = new_ht_table == NULL;
        if (*malloc_failed)
            return DICT_ERR;
    } else
        new_ht_table = (dictEntry **) zcalloc(newsize * sizeof(dictEntry *));
...
dictEntry *dictAddRaw(dict *d, void *key, dictEntry **existing) {
    ...
    entry = (dictEntry *) zmalloc(sizeof(*entry) + metasize);
    if (metasize > 0) {
